...
public class ArquivoProdutos {
    ...
    public void atualizar(Produto produtoParaAtualizar, Produto produtoAtualizado) {
        ArrayList<Produto> produtos = ler(); // Ler todos os produtos do arquivo

        // Encontrar o produto a ser atualizado
        for (int i = 0; i < produtos.size(); i++) {
            Produto produto = produtos.get(i);
            if (produto.equals(produtoParaAtualizar)) { // Implemente um método equals() em Produto para verificar
                                                        // igualdade
    ...
    private void reescreverArquivo(ArrayList<Produto> produtos) {
    ...
    public void excluir(Produto produtoParaExcluir) {
        ArrayList<Produto> produtos = ler(); // Ler todos os produtos do arquivo
        System.out.println("Produtos antes da exclusao: " + produtos.size());

        String categoriaExcluirProduto = produtoParaExcluir.getCategoria();
        String marcaExcluirProduto = produtoParaExcluir.getMarca();
        String modeloExcluirProduto = produtoParaExcluir.getModelo();
        double precoExcluirProduto = produtoParaExcluir.getPreco();
        int qtdExcluirProduto = produtoParaExcluir.getQtd();

        // Encontrar o produto a ser excluído
        produtos.removeIf(produto -> 
        produto.getCategoria().equals(produtoParaExcluir.getCategoria()) &&
        produto.getMarca().equals(produtoParaExcluir.getMarca()) &&
        produto.getModelo().equals(produtoParaExcluir.getModelo()) &&
        produto.getPreco() == produtoParaExcluir.getPreco() &&
        produto.getQtd() == produtoParaExcluir.getQtd()
        );
                                   
        /*
        for (int i = 0; i < produtos.size(); i++) {
            if (produtos.get(i).getCategoria().equals(categoriaExcluirProduto)
                    && produtos.get(i).getMarca().equals(marcaExcluirProduto)
                    && produtos.get(i).getModelo().equals(modeloExcluirProduto)
                    && produtos.get(i).getPreco() == precoExcluirProduto
                    && produtos.get(i).getQtd() == qtdExcluirProduto) 
            {
                produtos.remove(i);
            }
        }
        */
        ...
        System.out.println("Produtos depois da exclusao: " + produtos.size());

        // Reescrever o arquivo excluindo o produto
        reescreverArquivo(produtos);
    }
